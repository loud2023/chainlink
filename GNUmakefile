...
.PHONY: chainlink-solana-build
chainlink-solana-build: ## Build & install the chainlink-solana binary.
	go build $(GOFLAGS) -o chainlink-solana ./core/services/relay/cmd/chainlink-solana
	rm -f $(GOBIN)/chainlink-solana
	cp chainlink-solana $(GOBIN)/chainlink-solana
